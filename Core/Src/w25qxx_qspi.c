...
static uint8_t W25QXX_Wait_Busy_Timeout(uint32_t timeout_ms)
{
    uint32_t t0 = HAL_GetTick();
    while (1)
    {
        uint8_t sr1 = w25qxx_ReadSR(W25X_ReadStatusReg1);
        if ((sr1 & W25X_SR_WIP) == 0) {
            return w25qxx_OK;
        }
...
//		W25QXX_Wait_Busy();
	if (result == w25qxx_OK)
	    result = W25QXX_Wait_Busy_Timeout(2000);  // sector erase может быть десятки-сотни мс
...
//		W25QXX_Wait_Busy();
	if (result == w25qxx_OK)
	    result = W25QXX_Wait_Busy_Timeout(100);   // page program обычно << 20ms
